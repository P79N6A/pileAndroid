...
public class QRBizImpl implements QRBiz {

    @Override
    public void scan(String url, final String data, final QRListener listener) {
    ...
    @Override
    public void stop_scan(String url, final String phone, final String str, final QRListener listener) {
        StringRequest request = new StringRequest(Method.POST, url,
                new Listener<String>() {//参数设置
                    @Override
                    public void onResponse(String jsonstring) {//返回数据成功监听
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                obj.addProperty("packageData", str);
     ...
     * @return
     */
    public List<Object> isLegalOrNot(Context context, boolean legalOrNot, int buttonStyleLastId, String charging_type, String inputString,
        ...
        if(buttonStyleLastId==0){//自动充满
            ...
            if (buttonStyleLastId == 4) {//按进度冲,数值最多可填写2位，且只能是整数
                ...
                if (inputString.length() > 2) {
                    ToastUtil.showToast(context,"最多2位数值！");
                    legalOrNot = false;
                } else {
                    try {
                ...
                if (inputString.length() > 3) {//这里是3不是2，因为小数点也占一位
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        legalOrNot = false;
                        ToastUtil.showToast(context,"充电预设值不正确！");
                    }
                }
                ...
                if (inputString.length() > 3) {
                    ToastUtil.showToast(context,"最多3位数值！");
                    legalOrNot = false;
                } else {
                    try {
                        int inputInt = Integer.parseInt(inputString);
